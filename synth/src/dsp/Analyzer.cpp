...
void Analyzer::connectInput(BaseModule* source, size_t index){
    if ( !source ) return ;
...
void Analyzer::disconnectInput(BaseModule* source, size_t index){
    if ( !source ) return ;
...
const std::vector<std::pair<BaseModule*, size_t>>& Analyzer::getSources() const {    
    return sources_ ;
}